...
public class OrderService : IOrderService
{
    ...
    private readonly IRepository<UserAddress> _userAdressRepository;
    ...
    public OrderService(IRepository<Cart> cartRepository, IRepository<CartItem> cartItemRepository,
        IRepository<UserAddress> userAdressRepository,
        IRepository<Order> orderRepository, IRepository<OrderItem> orderItemRepository,
        ITaxService taxService, ICouponService couponService,
        ICartService cartService, IShippingPriceService shippingPriceService,
        ICurrencyService currencyService, IMediator mediator)
    {
    ...
    public async Task<Result<Order>> CreateOrder(long cartId, string paymentMethod, decimal paymentFeeAmount, OrderStatus orderStatus = OrderStatus.New)
    {
        ...
        if (shippingData.ShippingAddressId == 0)
        {
            ...
            _userAdressRepository.Add(userAddress);

            shippingAddress = address;
        }
        else
        {
            shippingAddress = _userAdressRepository.Query().Where(x => x.Id == shippingData.ShippingAddressId).Select(x => x.Address).First();
        }
        ...
        {
            billingAddress = _userAdressRepository.Query().Where(x => x.Id == shippingData.BillingAddressId).Select(x => x.Address).First();
        }
    ...
    {
        var cart = _cartRepository.Query()
       ...
       if (!checkingDiscountResult.Succeeded)
       {
        ...
        // thêm địa chỉ thanh toán đơn hàng
        ...
        {
            // kiểm tra đơn hàng cho phép order không?
        ...
        var vendorIds = cart.cartItems.Where(x => x.Product.VendorId.HasValue).Select(x => x.Product.VendorId.Value).Distinct();
        if (vendorIds.Any())
        {
        ...
        foreach (var vendorId in vendorIds)
        {
            ...
            foreach (var cartItem in cart.cartItems.Where(x => x.Product.VendorId == vendorId))
            {
                var taxPercent = await _taxService.GetTaxPercent(cartItem.Product.TaxClassId, shippingAddress.CountryId, shippingAddress.StateOrProvinceId, shippingAddress.ZipCode);
                var productPrice = cartItem.Product.Price;
                if (cart.IsProductPriceIncludeTax)
    ...
    public async Task<decimal> GetTax(long cartId, string countryId, long stateOrProvinceId, string zipCode)
    {
        ...
        foreach (var cartItem in cartItems)
        {
            if (cartItem.TaxClassId.HasValue)
            {
                ...
                taxAmount = taxAmount + cartItem.Quantity * cartItem.ProductPrice * taxRate / 100;
            }
        }
    ...
    public async Task<OrderTaxAndShippingPriceVm> UpdateTaxAndShippingPrices(long cartId, TaxAndShippingPriceRequestVm model)
    {
        ...
        {
            address = await _userAdressRepository.Query().Where(x => x.Id == model.ExistingShippingAddressId).Select(x => x.Address).FirstOrDefaultAsync();
            if (address == null)
            {
        ...
        {
            OrderAmount = (decimal)orderTaxAndShippingPrice.Cart.OrderTotal,
            ShippingAddress = address
        };
    ...
    private async Task<Result<ShippingPrice>> ValidateShippingMethod(string shippingMethodName, Address shippingAddress, Models.Cart cart)
    {
        var applicableShippingPrices = await _shippingPriceService.GetApplicableShippingPrices(new GetShippingPriceRequest
        {
            OrderAmount = cart.cartItems.Sum(x => x.Product.Price * x.Quantity),
