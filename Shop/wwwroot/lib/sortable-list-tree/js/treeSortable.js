	...
	initSorting() {
		...
		$(treeSelector).sortable({
			...
			sort(_, ui) {
				 ...
				 */
					//console.log("currentBranchEdge : " + currentBranchEdge);
				...
				newLevel = Math.max(lowerBound, Math.min(newLevel, upperBound));
					...
					//console.log("newLevelupdate : " + newLevel);
			...
			change(_, ui) {
				...
				/**  Store the original level. */
				//originalLevel = level;
				//menu.items.forEach(function (item) { item.children = []; });
				//menuItemTree = arrayToTree(menu.items);
			},
			stop(_, ui) {
				///**
				...
				//if (index > -1) {
				//	menu.items[index].displayOrder = originalIndex;
				//	if (level > 1) {
				//		var parentId = parseInt(ui.item.getParent().data("id"));
...
function displayOrder() {
	$("#tree li").each(function (i, el) {
		...
		var index = menu.items.findIndex(x => x.id == Id); // Id là duy nhất
		if (index > -1) {
			// mảng cũ chưa thay đổi bây giờ change vị trí 
